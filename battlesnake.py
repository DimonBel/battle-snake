...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    best_move = choose_best_move(my_head, my_body, safe_moves, board_width, board_height, snakes, food)
...
def choose_best_move(head, body, safe_moves, board_width, board_height, snakes, food):
    ...
    for move in safe_moves:
        ...
        space_available = count_reachable_spaces(new_head, body, board_width, board_height, snakes)
        score += space_available * 2
        
        # Avoid moves towards larger snakes' heads
        for snake in snakes:
            if snake["id"] != body[0] and len(snake["body"]) >= len(body):  # Larger or equal snake
    ...
    while queue and count < 100:  # Limit search to avoid timeout
        ...
        for move in ["up", "down", "left", "right"]:
            ...
            if (new_key not in visited and 
                is_safe_position(new_pos, my_body, board_width, board_height, snakes)):
